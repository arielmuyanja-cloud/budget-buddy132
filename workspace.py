...
VENDOR_DIRECTORY = {
    "asana": ("Asana", "Project Management"), "clickup": ("ClickUp", "Project Management"),
    "monday": ("monday.com", "Project Management"), "trello": ("Trello", "Project Management"),
    "notion": ("Notion", "Knowledge Management"), "slack": ("Slack", "Team Communication"),
    "zoom": ("Zoom", "Video Communication"), "loom": ("Loom", "Video Communication"),
    "canva": ("Canva", "Design"), "figma": ("Figma", "Design"), "adobe": ("Adobe", "Design"),
    "hubspot": ("HubSpot", "CRM / Marketing Automation"), "mailchimp": ("Mailchimp", "Email Marketing"),
    "semrush": ("Semrush", "SEO"), "ahrefs": ("Ahrefs", "SEO"),
    "google workspace": ("Google Workspace", "Productivity Suite"), "microsoft 365": ("Microsoft 365", "Productivity Suite"),
    "github": ("GitHub", "Developer Tools"), "vercel": ("Vercel", "Hosting / Deployment"),
    "render": ("Render", "Hosting / Deployment"), "aws": ("AWS", "Cloud Infrastructure"),
...
def recurring_candidates(user_id):
    ...
    for tx in transactions:
        ...
        if not key: continue
        groups.setdefault(key, {"raw": tx.description, "transactions": []})["transactions"].append(tx)
    ...
    for key, group in groups.items():
        ...
        candidates.append({"merchant_key": key, "raw_merchant": group["raw"], "vendor": vendor or group["raw"], "taxonomy": taxonomy,
                           "amount": round(avg, 2), "appearances": len(txs), "months": len(month_keys), "consecutive": consecutive,
                           "points": score, "evidence_strength": evidence_tier(score), "is_known": bool(vendor)})
    return candidates
    ...
    for i, a in enumerate(known):
        for b in known[i + 1:]:
            if a["taxonomy"] and a["taxonomy"] == b["taxonomy"]:
                score = evidence_score(data_months, a["vendor"], a["taxonomy"], True,
                                       a["amount"] == b["amount"])
                pairs.append((max(a["amount"], b["amount"]), a, b, score))
    if pairs:
        _, a, b, score = max(pairs, key=lambda x: x[0])
        return {"mode":"overlap", "title":f"{a['vendor']} + {b['vendor']}", "a":a, "b":b,
                "taxonomy":a["taxonomy"], "evidence_strength":evidence_tier(score), "score":score, "months":data_months,
                "message":f"Potential overlap: both tools are classified as {a['taxonomy']}. Transaction data cannot establish redundant functionality or active use."}
    unknown = [c for c in candidates if not c["is_known"]]
    if unknown:
        c = max(unknown, key=lambda x:x["amount"])
        return {"mode":"unknown", "candidate":c, "evidence_strength":c["evidence_strength"], "score":c["points"], "months":data_months,
                "message":"No defensible overlap was found. This is the largest unidentified recurring charge requiring review."}
...
def serialize_decision(d):
    return {"id":d.id,"tool_name":d.tool_name,"action":d.action,"savings_monthly":round(d.savings_monthly or 0,2),
            "risk_rating":d.risk_rating,"status":d.status,"active":d.active,
            "answers":{"q1":d.q1_client_workflow,"q2":d.q2_active_use,"q3":d.q3_important_files,"q4":d.q4_critical_integrations}}
...
def dashboard_totals(user_id):
    ...
    low = sum(d.savings_monthly for d in rows if d.risk_rating == "LOW" and d.status == "Verified" and d.active)
...
def workspace_home():
    ...
    return render_template("workspace.html", discovery=discovery, candidates=candidates, decisions=[serialize_decision(d) for d in decisions], totals=dashboard_totals(user_id), current_software_spend=current_spend)
...
def toggle_decision(decision_id):
    d = WorkspaceDecision.query.filter_by(id=decision_id,user_id=session["user_id"]).first_or_404()
    d.active = not d.active; db.session.commit()
    return jsonify({"success":True,"decision":serialize_decision(d),"totals":dashboard_totals(session["user_id"])})
